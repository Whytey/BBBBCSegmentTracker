...
`,is:"iron-a11y-announcer",properties:{/**
   ...
   */get _defaultPositionTarget(){var parent=(0,_bbbbcSegmentTracker.dom)(this).parentNode;if(parent&&parent.nodeType===Node.DOCUMENT_FRAGMENT_NODE){parent=parent.host}return parent},/**
...
var children;if("content"===element.localName||"slot"===element.localName){children=(0,_bbbbcSegmentTracker.dom)(element).getDistributedNodes()}else{// Use shadow root if possible, will check for distributed nodes.
children=(0,_bbbbcSegmentTracker.dom)(element.root||element).children}for(var i=0;i<children.length;i++){// Ensure method is always invoked to collect tabbable children.
...
this.__openedRaf=null},attached:function(){this.opened&&this._openedChanged(this.opened)},/**
   ...
   */prepare:function(){if(this.opened&&!this.parentNode){(0,_bbbbcSegmentTracker.dom)(document.body).appendChild(this)}},/**
   * Shows the backdrop.
   */open:function(){this.opened=!0},/**
   * Hides the backdrop.
   */close:function(){this.opened=!1},/**
   * Removes the backdrop from document body if needed.
   */complete:function(){if(!this.opened&&this.parentNode===document.body){(0,_bbbbcSegmentTracker.dom)(this.parentNode).removeChild(this)}},_onTransitionend:function(event){if(event&&event.target===this){this.complete()}},/**
...
if(!active||!1===active instanceof Element){active=document.body}while(active.root&&(0,_bbbbcSegmentTracker.dom)(active.root).activeElement){active=(0,_bbbbcSegmentTracker.dom)(active.root).activeElement}return active},/**
...
if(z1===z1){z=z1}}return z},/**
   ...
   * @param {!Event} event
   * @private
   */_onCaptureClick:function(event){var i=this._overlays.length-1;if(-1===i)return;var path=/** @type {!Array<!EventTarget>} */(0,_bbbbcSegmentTracker.dom)(event).path,overlay;// Check if clicked outside of overlay.
while((overlay=/** @type {?} */this._overlays[i])&&this._overlayInPath(path)!==overlay){overlay._onCaptureClick(event);if(overlay.allowClickThrough){i--}else{break}}},/**
   ...
   * @param {!Event} event
   * @private
   */_onCaptureKeyDown:function(event){var overlay=/** @type {?} */this.currentOverlay();if(overlay){if(_bbbbcSegmentTracker.IronA11yKeysBehavior.keyboardEventMatchesKeys(event,"esc")){overlay._onCaptureEsc(event)}else if(_bbbbcSegmentTracker.IronA11yKeysBehavior.keyboardEventMatchesKeys(event,"tab")){overlay._onCaptureTab(event)}}},/**
...
var contentElements,distributedNodes,contentIndex,nodeIndex;if(element.contains(child)){return!0}contentElements=(0,_bbbbcSegmentTracker.dom)(element).querySelectorAll("content,slot");for(contentIndex=0;contentIndex<contentElements.length;++contentIndex){distributedNodes=(0,_bbbbcSegmentTracker.dom)(contentElements[contentIndex]).getDistributedNodes();for(nodeIndex=0;nodeIndex<distributedNodes.length;++nodeIndex){// Polymer 2.x returns slot.assignedNodes which can contain text nodes.
...
var target=(0,_bbbbcSegmentTracker.dom)(event).rootTarget;if("touchmove"!==event.type&&lastRootTarget!==target){lastRootTarget=target;lastScrollableNodes=_getScrollableNodes((0,_bbbbcSegmentTracker.dom)(event).path)}// Prevent event if no scrollable nodes.
...
type:Object},/**
   ...
   * @return {!Node}
   */get _focusNode(){return this._focusedChild||(0,_bbbbcSegmentTracker.dom)(this).querySelector("[autofocus]")||this},/**
...
if(this.opened){this._openedChanged(this.opened)}this._observer=(0,_bbbbcSegmentTracker.dom)(this).observeNodes(this._onNodesChange)},detached:function(){(0,_bbbbcSegmentTracker.dom)(this).unobserveNodes(this._observer);this._observer=null;for(var cb in this.__rafs){if(null!==this.__rafs[cb]){cancelAnimationFrame(this.__rafs[cb])}}this.__rafs={};this._manager.removeOverlay(this);// We got detached while animating, ensure we show/hide the overlay
...
var activeElement=this._manager.deepActiveElement;if(activeElement===document.body||(0,_bbbbcSegmentTracker.dom)(this).deepContains(activeElement)){this.__restoreFocusNode.focus()}}this.__restoreFocusNode=null;this._focusNode.blur();this._focusedChild=null}},/**
   ...
   * @param {!Event} event
   * @protected
   */_onCaptureFocus:function(event){if(!this.withBackdrop){return}var path=(0,_bbbbcSegmentTracker.dom)(event).path;if(-1===path.indexOf(this)){event.stopPropagation();this._applyFocus()}else{this._focusedChild=path[0]}},/**
...
if(0<=(0,_bbbbcSegmentTracker.dom)(event).path.indexOf(this)){return}switch(this.scrollAction){case"lock":// NOTE: scrolling might happen if a scroll event is not cancellable, or
...
document.documentElement.scrollTop=document.body.scrollTop=this.__scrollTop;document.documentElement.scrollLeft=document.body.scrollLeft=this.__scrollLeft}}};/**
     ...
     @polymerBehavior
    */_exports.IronOverlayBehaviorImpl=IronOverlayBehaviorImpl;const IronOverlayBehavior=[IronFitBehavior,_bbbbcSegmentTracker.IronResizableBehavior,IronOverlayBehaviorImpl];/**
                                                                                                       ...
                                                                                                       */ /**
                                                                                                           ...
                                                                                                           */ /**
                                                                                                               ...
                                                                                                               */_exports.IronOverlayBehavior=IronOverlayBehavior;var ironOverlayBehavior={IronOverlayBehaviorImpl:IronOverlayBehaviorImpl,IronOverlayBehavior:IronOverlayBehavior};_exports.$ironOverlayBehavior=ironOverlayBehavior;const PaperSpinnerBehavior={properties:{/**
...
`;template.setAttribute("strip-whitespace","");/**
                                               ...
                                               @hero hero.svg
                                               @demo demo/index.html
                                               */(0,_bbbbcSegmentTracker.Polymer)({_template:template,is:"paper-spinner-lite",behaviors:[PaperSpinnerBehavior]});var currentToast=null;/**
...
`,is:"paper-toast",behaviors:[IronOverlayBehavior],properties:{/**
   ...
   * @property visible
   * @deprecated
   */get visible(){_bbbbcSegmentTracker.Base._warn("`visible` is deprecated, use `opened` instead");return this.opened},/**
   ...
   */show:function(properties){if("string"==typeof properties){properties={text:properties}}for(var property in properties){if(0===property.indexOf("_")){_bbbbcSegmentTracker.Base._warn("The property \""+property+"\" is private and was not set.")}else if(property in this){this[property]=properties[property]}else{_bbbbcSegmentTracker.Base._warn("The property \""+property+"\" is not valid.")}}this.open()},/**
...
if(e&&e.target===this&&"opacity"===e.propertyName){if(this.opened){this._finishRenderOpened()}else{this._finishRenderClosed()}}},/**
   ...
   */_onFitIntoChanged:function(fitInto){this.positionTarget=fitInto}/**
    ...
    `}static get properties(){return{step2Body:Object,addedUser:Object,__step1Succeeded:{type:Boolean,value:!1},__step1Errored:{type:Boolean,value:!1},__step2Succeeded:{type:Boolean,value:!1},__step2Errored:{type:Boolean,value:!1}}}step1Response(){// We have come into the page with some params, if they didn't include an error, store the connection.
var qp=this.$.location.queryParams;if(!qp.error){this.__step1Succeeded=!0;this.$.step2Ajax.body={code:qp.code};this.$.step2Ajax.generateRequest()}else{this.__step1Errored=!0}}processStep2Response(event){console.log("Step 2 responded");this.__step2Succeeded=!0;this.addedUser=event.detail.response;console.log(this.addedUser)}processStep2Error(){console.log("Step 2 errored");this.__step2Errored=!0}json(s){return JSON.stringify(s,null,2)}}window.customElements.define("connect-view",ConnectView)});