...
if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});class MyConfig extends PolymerElement{static get template(){return html`
    ...
    `}static get properties(){return{config:{type:Object,reflectToAttribute:!0,readOnly:!0,notify:!0}}}configChanged(){var config=this.$.appconfig.byKey("config");this._setConfig(JSON.parse(config))}}window.customElements.define("my-config",MyConfig);class ConnectButton extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
    ...
    `}__getRedirectURL(){var url=new URL(document.location);url.pathname="connect-view";console.log(url.toString());var params={redirect_url:url.toString()};return params}}window.customElements.define("connect-button",ConnectButton);const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`<iron-iconset-svg name="my-icons" size="24">
  <svg>
    <defs>
      <g id="arrow-back">
...
</iron-iconset-svg>`;document.head.appendChild($_documentContainer.content);// preventable, allowing for better scrolling performance.
