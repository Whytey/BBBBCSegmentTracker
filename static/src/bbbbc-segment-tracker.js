...
if(30<Math.abs(this._initScrollTop-scrollTop)||10<absDScrollTop){if(isScrollingDown&&scrollTop>=maxHeaderTop){top=maxHeaderTop}else if(!isScrollingDown&&scrollTop>=this._dHeight){top=this.condenses&&!this.fixed?this._dHeight:0}var scrollVelocity=dScrollTop/(now-this._lastTimestamp);this.style.transitionDuration=this._clamp((top-lastTop)/scrollVelocity,0,300)+"ms"}else{top=this._top}}}if(0===this._dHeight){progress=0<scrollTop?1:0}else{progress=top/this._dHeight}if(!forceUpdate){this._lastScrollTop=scrollTop;this._top=top;this._wasScrollingDown=isScrollingDown;this._lastTimestamp=now}if(forceUpdate||progress!==this._progress||lastTop!==top||0===scrollTop){this._progress=progress;this._runEffects(progress,top);this._transformHeader(top)}},/**
   ...
   */setUp:function setUp(config){var fx={},scalar=parseFloat(config.scalar);fx.background=this._getDOMRef("background");fx.backgroundFrontLayer=this._getDOMRef("backgroundFrontLayer");fx.backgroundRearLayer=this._getDOMRef("backgroundRearLayer");fx.deltaBg=fx.backgroundFrontLayer.offsetHeight-fx.background.offsetHeight;if(0===fx.deltaBg){if(isNaN(scalar)){scalar=.8}fx.deltaBg=(this._dHeight||0)*scalar}else{if(isNaN(scalar)){scalar=1}fx.deltaBg=fx.deltaBg*scalar}this._fxParallaxBackground=fx},/** @this {ParallaxBackground} */run:function run(p,y){var fx=this._fxParallaxBackground;this.transform("translate3d(0px, "+fx.deltaBg*Math.min(1,p)+"px, 0px)",fx.backgroundFrontLayer);if(fx.backgroundRearLayer){this.transform("translate3d(0px, "+fx.deltaBg*Math.min(1,p)+"px, 0px)",fx.backgroundRearLayer)}},/** @this {ParallaxBackground} */tearDown:function tearDown(){delete this._fxParallaxBackground}});/**
    ...
    <slot></slot>
`,is:"app-toolbar"});Polymer({is:"iron-location",_template:null,properties:{/**
...
var fullNormalizedHref=new URL(normalizedHref,this.__location.href).href;return fullNormalizedHref},_makeRegExp:function(urlSpaceRegex){return RegExp(urlSpaceRegex)}});Polymer({is:"iron-query-params",_template:null,properties:{/**
...
resolved=(location.origin||location.protocol+"//"+location.host)+resolved}return resolved}});class IronSelection{/**
...
</custom-style>`;template$2.setAttribute("style","display: none;");document.head.appendChild(template$2.content);Polymer({is:"paper-icon-button",_template:html`
    <style>
      :host {
        display: inline-block;
        position: relative;
...
if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`<iron-iconset-svg name="my-icons" size="24">
  <svg>
    <defs>
      <g id="arrow-back">
...
</iron-iconset-svg>`;document.head.appendChild($_documentContainer.content);class ConnectButton extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
    ...
    `}}window.customElements.define("connect-button",ConnectButton);// preventable, allowing for better scrolling performance.
setPassiveTouchGestures(!0);// Set Polymer's root path to the same value we passed to our service worker
// in `index.html`.
setRootPath(MyAppGlobals.rootPath);class BBBBCSegmentTracker extends PolymerElement{static get template(){return html`
      <style>
        :host {
    ...
    `}static get properties(){return{page:{type:String,reflectToAttribute:!0,observer:"_pageChanged"},routeData:Object,subroute:Object}}static get observers(){return["_routePageChanged(routeData.page)"]}_routePageChanged(page){// Show the corresponding page according to the route.
//
// If no page was found in the route data, page will be an empty string.
// Show 'view1' in that case. And if the page doesn't exist, show 'view404'.
if(!page){this.page="home-view"}else if(-1!==["home-view","members-view","challenges-view"].indexOf(page)){this.page=page}else{this.page="view404"}// Close a non-persistent drawer when the page & route are changed.
if(!this.$.drawer.persistent){this.$.drawer.close()}}_pageChanged(page){// Import the page component on demand.
//
// Note: `polymer build` doesn't like string concatenation in the import
// statement, so break it up.
switch(page){case"home-view":new Promise((res,rej)=>_require.default(["./home-view.js"],res,rej)).then(bundle=>bundle&&bundle.$homeView||{});break;case"members-view":new Promise((res,rej)=>_require.default(["./members-view.js"],res,rej)).then(bundle=>bundle&&bundle.$membersView||{});break;case"challenges-view":new Promise((res,rej)=>_require.default(["./challenges-view.js"],res,rej)).then(bundle=>bundle&&bundle.$challengesView||{});break;case"view404":new Promise((res,rej)=>_require.default(["./my-view404.js"],res,rej)).then(bundle=>bundle&&bundle.$myView404||{});break;}}}window.customElements.define("bbbbc-segment-tracker",BBBBCSegmentTracker);const $_documentContainer$1=document.createElement("template");$_documentContainer$1.innerHTML=`<dom-module id="shared-styles">
  <template>
    <style>
      .card {
        margin: 24px;
